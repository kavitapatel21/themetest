...
/*		if ( ! is_admin() || defined('UWA_DOING_AJAX') ) {
	...
	public function uwa_add_product_to_cart() {
		...
		if (!is_admin()) {
			...
			if (!empty($_GET['pay-uwa-auction'])) {
				...
				wp_safe_redirect(remove_query_arg(array('pay-uwa-auction', 'quantity', 'product_id'), 
					wc_get_checkout_url()));				
				exit;
			}
		}
	...
	*/	
	...
	function uwa_auction_footer_text(){
		...
		$footer_text1 = __('Powered by', "ultimate-woocommerce-auction");
		$footer_text2 = __('Ultimate Auction', "ultimate-woocommerce-auction");
